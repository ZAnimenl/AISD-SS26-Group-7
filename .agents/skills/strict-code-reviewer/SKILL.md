...
If a change relies on version-sensitive library/API behavior, check whether it was verified against current documentation, preferably through Context7 MCP or official docs. Flag stale, hallucinated, or unsupported API usage, but do not let external documentation override the project specs or API alignment document.
