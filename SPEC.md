...
- **REQ-11b** The system shall allow an administrator to configure an assessment so that it contains one task from each of the four supported task categories.
...
- **REQ-18b** Each assessment should contain one task from each of the four primary task categories.
...
- Each assessment should contain the four primary task categories: frontend UI extension, REST API development, database query/schema work, and bug fixing.
...
Accepted when an administrator can create an assessment based on a shared runnable prototype, select or generate one task for each of the four supported task categories, review/edit/approve test cases, and activate the assessment.
...
Accepted when an administrator can select a task category, request an LLM-generated draft task and test cases, review or edit the generated content, and approve it before publication.
