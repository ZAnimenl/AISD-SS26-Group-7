...
`Backend/Backend/appsettings.Development.json` does not contain database or seed
administrator credentials, but the current checkout does include dev-only
Google OAuth and SMTP values there for the private course demo. Treat those
values as private, rotate/remove them before making the repository public, and
prefer `.env.local`, environment variables, .NET user-secrets, or deployment
secrets for any replacement credentials. ASP.NET Core reads environment
...
- `/student/assessments/[assessmentId]/reflection`
- `/student/assessments/[assessmentId]/review`
- `/student/results`
- `/admin/dashboard`
- `/admin/assessments`
...
- auth login/logout/me, three-step code registration, forgot/change password,
  and Google OAuth callback handling
...
Runtime config is exposed through `GET /api/v1/config`. The current backend
advertises Bearer-token auth, roles `student` and `administrator`, feature
flags including `multi_file_workspace_enabled=true`, `real_sandbox_enabled`
based on Docker runtime probing, and supported language values `python`,
`javascript`, `typescript`, `html`, and `sql`.
